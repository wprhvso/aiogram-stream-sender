...
**Ритм.** `send_interval`, `edit_interval`, `delete_interval` (по 1.0 с) и `action_interval` (4.0 с). Это минимальные паузы между действиями **одного вида в одном чате**. Виды независимы: send не мешает edit. Telegram считает лимиты примерно так же.
